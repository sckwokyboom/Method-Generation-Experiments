...
from pipeline.call_graph import (
    build_call_graph,
    build_edge_counts,
    canonical_vertex_id,
    canonicalize_invocation_signature,
    flatten_vertices,
    load_call_graph,
    save_call_graph,
)
...
def test_vertex_id_matches_invocation_signature_under_whitespace():
    # Whitespace in param types (e.g. generics "Map<K, V>") must not produce
    # a vertex_id that differs from the canonicalized invocation signature —
    # otherwise real edges get silently dropped as "external".
    vid = canonical_vertex_id(
        class_fqn="com.example.Foo",
        method_name="bar",
        parameter_types=["java.util.Map<K, V>"],
        return_type="void",
    )
    sig = canonicalize_invocation_signature(
        "com.example.Foo::bar(java.util.Map<K, V>) -> void"
    )
    assert vid == sig
